...
├── popup/
│   ├── popup.html         # ポップアップUI
│   ├── popup.js           # ポップアップロジック（言語切り替え含む）
│   └── popup.css          # ポップアップスタイル
├── options/
│   ├── options.html       # オプションページUI
│   ├── options.js         # オプションページロジック（多言語対応）
...
### ポップアップ
- ヘッダー: タイトル + 言語切り替え（セグメントコントロール: JP/EN）
- 有効/無効トグル（ラベル付き）
- 残り時間の表示（プログレスバー付き）
- 使用時間/上限時間の表示
- 上限時間のクイック変更（+/- ボタン）
